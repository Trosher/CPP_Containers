...
template <typename Key>
std::pair<typename Set<Key>::Nodes *, bool> Set<Key>::FindWrite_(
    const value_type &value, bool passed) {
...
template <typename Key>
std::pair<typename Set<Key>::Nodes *, bool> Set<Key>::FindRead_(
...
template <typename Key>
void Set<Key>::FixUp_(Nodes *node) {
  if (node != nullptr) {
...
template <typename Key>
void Set<Key>::Rotate_(Nodes *node, const bool left) {
...
template <typename Key>
void Set<Key>::RemoveNode_(Nodes *node) {
...
template <typename Key>
bool Set<Key>::FixDown_(Nodes *node) {
  bool result = false;
  Nodes *parent = node->parent;
...
template <typename Key>
bool Set<Key>::IsNull_(Nodes *node, int flag) const {
  bool res;
  if (!flag)
    res = node == nullptr;
  else if (flag == 1)
...
template <typename Key>
void Set<Key>::RemoveSet_(Nodes *node) {
